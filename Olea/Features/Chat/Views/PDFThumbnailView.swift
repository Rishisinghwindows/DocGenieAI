...
struct PDFThumbnailView: View {
    ...
    var body: some View {
        ...
        // Snapshot the URL at body-time via capture list so the .task closure never
        // re-touches the SwiftData @Model object after a possible refault.
        .task { [url = document?.fileURL] in
            ...
            let thumb = await Task.detached(priority: .utility) { () -> UIImage? in
                ...
                return page.thumbnail(of: CGSize(width: 112, height: 144), for: .mediaBox)
            }.value
            if let thumb { thumbnail = thumb }
        }
    }
}
