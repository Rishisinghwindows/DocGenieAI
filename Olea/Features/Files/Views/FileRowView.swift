...
struct FileRowView: View {
    ...
    var body: some View {
        ...
        // Capture-list snapshot evaluates the @Model getters at body-time on the main
        // actor while `file` is guaranteed valid. The .task closure then operates only
        // on value-type locals — never re-touches the persistent object after it may
        // have been faulted/deleted (which is what crashed the production build).
        .task { [ext = file.fileExtension.lowercased(), url = file.fileURL] in
            guard let url, ext == "pdf" || ["jpg", "jpeg", "png", "heic"].contains(ext) else { return }
            if ext == "pdf" {
