    ...
    func extractTextFromImage(_ image: UIImage) async throws -> String {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            do {
                ...
                // Completion handler above will resume the continuation
            } catch {
                // perform(_:) threw before invoking the completion handler,
                // so the continuation has not been resumed yet — safe to resume here.
                // Note: VNImageRequestHandler.perform is synchronous; if it throws,
                // the request's completion handler is guaranteed NOT to have been called.
                continuation.resume(throwing: error)
            }
        }
    }
