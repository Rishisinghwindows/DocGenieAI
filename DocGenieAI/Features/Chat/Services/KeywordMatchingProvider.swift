...
final class KeywordMatchingProvider: AIResponseProvider {
    ...
    private static let greetingResponses = [
        "Hello! I'm DocSage, your AI document assistant. I can help you with scanning, PDF tools, converting files, and extracting text. What would you like to do?",
        "Hey there! I'm DocSage. I can scan documents, merge PDFs, extract text, and much more. How can I help?",
        "Hi! Welcome to DocSage. I've got 23 tools ready to help with your documents. What do you need?",
        "Hello! DocSage here. Whether it's scanning, converting, or editing PDFs — I've got you covered. What's on your mind?",
    ]

    private static let fallbackResponses = [
        "I can help you with document tasks! Try asking me to scan, merge, convert, compress, or extract text. Or tap a quick action below.",
        "I'm best at document tasks like scanning, merging PDFs, or extracting text. Try one of those, or browse all tools below!",
        "Not sure I caught that. I can help with document tasks like scanning, PDF editing, and conversion. Pick a quick action or ask about a specific tool!",
        "I specialize in document tasks — scanning, merging, compressing, converting, and extracting text. What would you like to try?",
    ]
    ...
    func generateResponse(for input: String, conversationHistory: [ChatMessage]) async throws -> AIResponse {
        ...
        } else if lower.contains("hello") || lower.contains("hi") || lower.contains("hey") || lower.contains("good morning") || lower.contains("good afternoon") || lower.contains("good evening") {
            response = Self.greetingResponses.randomElement()!
            badge = nil
            chatActions = [
        ...
        // MARK: General Knowledge (ChatGPT-like)

        } else if lower.contains("what time") || lower.contains("what's the time") || lower.contains("current time") {
            let time = Date().formatted(.dateTime.hour().minute())
            response = "It's currently **\(time)**."
            badge = nil

        } else if lower.contains("what date") || lower.contains("today's date") || lower.contains("what day") {
            let date = Date().formatted(.dateTime.weekday(.wide).month(.wide).day().year())
            response = "Today is **\(date)**."
            badge = nil

        } else if lower.contains("who are you") || lower.contains("what are you") || lower.contains("your name") {
            response = "I'm **DocSage**, your AI document assistant. I can scan, edit, convert, and analyze documents — all on-device, fully private. I also remember your preferences across conversations. Try asking me to compress a PDF or summarize a document!"
            badge = nil

        } else if lower.contains("thank") || lower.contains("thanks") || lower.contains("appreciate") {
            let thanks = ["You're welcome! Let me know if you need anything else.", "Happy to help! Anything else I can do?", "Glad I could help! Feel free to ask anytime.", "No problem! I'm here whenever you need me."]
            response = thanks.randomElement()!
            badge = nil

        } else if lower.contains("how are you") || lower.contains("how's it going") || lower.contains("what's up") {
            let replies = ["I'm doing great, thanks for asking! Ready to help with your documents.", "All systems running smoothly! What can I help you with?", "I'm good! Got 22 tools ready and waiting. What do you need?"]
            response = replies.randomElement()!
            badge = nil

        } else if lower.contains("joke") || lower.contains("funny") || lower.contains("make me laugh") {
            let jokes = [
                "Why did the PDF go to therapy? It had too many issues to compress. 📄",
                "I tried to merge two PDFs but they couldn't agree on anything. Classic document conflict!",
                "What did the OCR say to the blurry image? \"I can't read you right now.\" 😄",
                "Why don't PDFs ever get lost? Because they always have page numbers! 📖",
            ]
            response = jokes.randomElement()!
            badge = nil

        } else if lower.contains("weather") {
            response = "I don't have access to weather data since I work entirely offline. Try asking Siri or checking the Weather app! But I'm great with documents — want to scan or convert something?"
            badge = nil

        } else if lower.contains("calculate") || lower.contains("math") || lower.contains("what is") && (lower.contains("+") || lower.contains("-") || lower.contains("*") || lower.contains("/") || lower.contains("plus") || lower.contains("minus") || lower.contains("times")) {
            response = "I'm focused on document tasks, so I can't do math calculations. But I can extract numbers from your documents using OCR! Want to try?"
            badge = nil
            chatActions = [
                ChatAction(label: "OCR Extract", icon: "text.viewfinder", actionType: .openTool, toolId: "OCR Text"),
            ]

        } else if lower.contains("can you") || lower.contains("are you able") || lower.contains("do you support") {
            response = "I can help with **22 document tools**: scanning, merging, compressing, locking, signing, OCR, summarizing, translating, and more. I also do voice notes, receipt parsing, and remember your preferences. Type `/help` to see all commands!"
            badge = nil

        } else if lower.contains("bye") || lower.contains("goodbye") || lower.contains("see you") || lower.contains("later") {
            let byes = ["Goodbye! Your documents are safe with me. 👋", "See you later! I'll remember your preferences for next time.", "Bye! Come back anytime you need document help."]
            response = byes.randomElement()!
            badge = nil

        } else if lower.contains("help") || lower.contains("what can you do") || lower.contains("features") {
            response = """
            Here's what I can do:

            📄 **Document Tools** — scan, merge, split, compress, lock, sign, rotate, watermark, crop, add page numbers
            🔍 **AI Analysis** — OCR, summarize, ask questions, translate
            🔄 **Convert** — images→PDF, doc→PDF, PDF→images, PDF→text
            ✍️ **Text Actions** — rewrite formal/casual, fix grammar, bullet points, expand
            🎙️ **Voice Notes** — record + transcribe
            🧾 **Smart Scan** — auto-detect receipts and business cards
            🧠 **Memory** — I remember your preferences
            ...
            // True fallback — still helpful
            response = Self.fallbackResponses.randomElement()!
            badge = nil
            chatActions = [
    ...
    private func handleDocumentContextRequest(input: String) -> AIResponse? {
        ...
        // Detect summarize intent — checked BEFORE general questions so "summarize" isn't swallowed
