...
final class FoundationModelsProvider: AIResponseProvider {
    ...
    You are DocSage, a friendly AI document assistant inside the DocSage app.
    ...
    func generateResponse(for input: String, conversationHistory: [ChatMessage]) async throws -> AIResponse {
        ...
        do {
            ...
            let cleanText = Self.stripToolCallText(from: response.content)
            return AIResponse(text: cleanText, toolBadge: badge, actions: actions)
        } catch {
    ...
    func streamResponse(
        ...
        do {
            ...
            for try await partial in stream {
                ...
                onPartialUpdate(Self.stripToolCallText(from: finalText))
            }

            let actions = extractActionsFromSession(session)
            let badge = determineBadge(from: actions)
            let cleanText = Self.stripToolCallText(from: finalText)
            return AIResponse(text: cleanText, toolBadge: badge, actions: actions)
        } catch {
            throw AIServiceError.responseGenerationFailed(underlying: error)
        }
    ...
    /// Strip raw tool call text that the LLM sometimes includes in the response
    private static func stripToolCallText(from text: String) -> String {
        var cleaned = text
        // Remove suggestTool(...) patterns
        let toolCallPattern = #"suggestTool\s*\([^)]*\)\s*"#
        if let regex = try? NSRegularExpression(pattern: toolCallPattern, options: [.dotMatchesLineSeparators]) {
            cleaned = regex.stringByReplacingMatches(in: cleaned, range: NSRange(cleaned.startIndex..., in: cleaned), withTemplate: "")
        }
        // Remove navigateTab(...) patterns
        let navPattern = #"navigateTab\s*\([^)]*\)\s*"#
        if let regex = try? NSRegularExpression(pattern: navPattern, options: [.dotMatchesLineSeparators]) {
            cleaned = regex.stringByReplacingMatches(in: cleaned, range: NSRange(cleaned.startIndex..., in: cleaned), withTemplate: "")
        }
