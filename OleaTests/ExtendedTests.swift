...
final class NavigationRouterTests: XCTestCase {
    ...
    func testResetCurrentTab_home() {
        router.selectedTab = .inbox
        router.inboxPath.append("test")
        router.resetCurrentTab()
        XCTAssertTrue(router.inboxPath.isEmpty)
    }
...
final class AppTabTests: XCTestCase {
    ...
    func testRawValues() {
        ...
        XCTAssertEqual(AppTab.files.rawValue, "files")
    ...
    func testIdentifiable() {
        XCTAssertEqual(AppTab.inbox.id, "inbox")
        XCTAssertEqual(AppTab.chat.id, "chat")
    }

    func testTitles() {
        XCTAssertEqual(AppTab.inbox.title, "Inbox")
        XCTAssertEqual(AppTab.tools.title, "Tools")
        XCTAssertEqual(AppTab.chat.title, "Ask")
        XCTAssertEqual(AppTab.settings.title, "Settings")
    }

    func testSystemImages() {
        XCTAssertEqual(AppTab.inbox.systemImage, "tray.full.fill")
        XCTAssertEqual(AppTab.tools.systemImage, "wrench.and.screwdriver.fill")
        XCTAssertEqual(AppTab.chat.systemImage, "sparkles")
...
final class ChatViewModelExtendedTests: XCTestCase {
    ...
    func testHandleAction_navigateTab_unknown() {
        ...
        viewModel.handleAction(action, coordinator: coordinator, router: router, context: container.mainContext)
        XCTAssertEqual(router.selectedTab, .inbox) // unchanged
    }
...
final class ToolItemExtendedTests: XCTestCase {
    ...
    func testSections() {
        XCTAssertEqual(ToolItem.scanner.section, "Scan")
        XCTAssertEqual(ToolItem.mergePDF.section, "Edit")
        XCTAssertEqual(ToolItem.imageToPDF.section, "Convert")
    ...
    func testAllSectionsPresent() {
        ...
        XCTAssertTrue(sections.contains("Scan"))
        XCTAssertTrue(sections.contains("Edit"))
        XCTAssertTrue(sections.contains("Enhance"))
        XCTAssertTrue(sections.contains("Protect"))
        XCTAssertTrue(sections.contains("Compare"))
        XCTAssertTrue(sections.contains("AI Intelligence"))
        XCTAssertTrue(sections.contains("Convert"))
        XCTAssertTrue(sections.contains("Share & Create"))
    }

    func testScanCount() {
        let scanTools = ToolItem.allCases.filter { $0.section == "Scan" }
        XCTAssertEqual(scanTools.count, 1)
    }

    func testEditCount() {
        let editTools = ToolItem.allCases.filter { $0.section == "Edit" }
        XCTAssertEqual(editTools.count, 6) // merge, split, extractPages, rotate, reorder, crop
    }

    func testEnhanceCount() {
        let enhanceTools = ToolItem.allCases.filter { $0.section == "Enhance" }
        XCTAssertEqual(enhanceTools.count, 5) // compress, pageNumbers, watermark, sign, metadata
    }

    func testProtectCount() {
        let protectTools = ToolItem.allCases.filter { $0.section == "Protect" }
        XCTAssertEqual(protectTools.count, 3) // lock, unlock, redact
    }

    func testCompareCount() {
        let compareTools = ToolItem.allCases.filter { $0.section == "Compare" }
        XCTAssertEqual(compareTools.count, 2) // comparePDF, batchProcess
    }

    func testAIIntelligenceCount() {
        let aiTools = ToolItem.allCases.filter { $0.section == "AI Intelligence" }
        XCTAssertEqual(aiTools.count, 6) // summarize, ask, translate, handwriting, ocr, formAutofill
    }

    func testConvertCount() {
        let convertTools = ToolItem.allCases.filter { $0.section == "Convert" }
        XCTAssertEqual(convertTools.count, 4)
    }

    func testShareAndCreateCount() {
        let shareCreate = ToolItem.allCases.filter { $0.section == "Share & Create" }
        XCTAssertEqual(shareCreate.count, 3) // templates, emailPDF, qrShare
    }
}
